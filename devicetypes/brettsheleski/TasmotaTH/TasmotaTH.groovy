metadata {
	definition(name: "TasmotaTH", namespace: "BrettSheleski", author: "Brett Sheleski", ocfDeviceType: "oic.d.smartplug") {
		capability "Polling"
		capability "Refresh"
	...
	tiles(scale: 2) {
        ...
        valueTile("temperature2", "temperature", width: 6, height: 2) {
			state "temperature", label: 'Temperature: ${currentValue}°', backgroundColor: "#ffffff"
		}  
        
        valueTile("temperature", "temperature", width: 3, height: 1) {
        ...
        valueTile("humidity", "humidity", width: 6, height: 2) {
		...
		main "temperature"
		details(["temperature2", "humidity", "refresh", "module", "topic", "version", "vcc", "hostname", "macAddress", "apSsid", "apMac"])
	}
...
def refreshCallback(physicalgraph.device.HubResponse response){
	...
	sendEvent(name : "temperature", value : response?.json?.StatusSNS?.AM2301?.Temperature)
	sendEvent(name : "humidity", value : response?.json?.StatusSNS?.AM2301?.Humidity)
